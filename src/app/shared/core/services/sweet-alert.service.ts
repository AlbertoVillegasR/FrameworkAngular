...
import * as Constants from '../../model/constants.model';
...
export class SweetAlertService {
    ...
    show(status: number, message: string, params?: Partial<{ title: string, content: string, buttonText: string }>) {
        if (message.toLocaleLowerCase() === 'success') message = Constants.STATUS_MESSAGES.Success;

        return sweetalert.fire({
            title: params?.title || this.getTitle(status),
            html: this.sanitizer.sanitize(SecurityContext.HTML, params?.content || message),
